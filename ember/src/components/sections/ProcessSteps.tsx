...
const steps: Step[] = [
  { icon: ClipboardCheck, title: "Discover", desc: "Goals, scope, timeline.", accent: "from-violet-500 to-fuchsia-500" },
  { icon: PenTool,        title: "Design",   desc: "Wireframes → hi-fi UX.", accent: "from-sky-500 to-cyan-400" },
  { icon: Code2,          title: "Build",    desc: "React / Expo + CI/CD.", accent: "from-emerald-500 to-lime-400" },
  { icon: Rocket,         title: "Ship",     desc: "QA, launch, support.",   accent: "from-amber-500 to-orange-500" },
];
          ...
          >
            <div className="relative rounded-2xl border border-black/10 bg-white/70 p-6 backdrop-blur dark:border-white/10 dark:bg-white/5">
              ...
              <div className="relative inline-flex">
                <div className={`grid h-12 w-12 place-items-center rounded-xl bg-gradient-to-br ${accent} text-white shadow-lg ring-1 ring-white/20`}>
                  <Icon className="h-6 w-6" />
                </div>
                <span className="absolute -right-3 -top-3 grid h-6 w-6 place-items-center rounded-full bg-black/80 text-xs font-semibold text-white shadow dark:bg-white/90 dark:text-black">
                  {i + 1}
                </span>
                {/* subtle glow */}
                <div className={`pointer-events-none absolute inset-0 -z-10 blur-xl opacity-40 bg-gradient-to-br ${accent}`} />
              </div>
              ...
              {/* micro progress bar */}
              <div className={`mt-5 h-1 w-1/2 rounded-full bg-gradient-to-r ${accent} opacity-70`} />
            </div>
          </div>
        ))}
      </div>
